...
def calcShannonEnt(dataSet):
    ...
    shannonEnt = 0.0
    for i in labelCounts:  # 求出每种类型的熵
        prob = float(labelCounts[i]) / numEntries  # 每种类型个数占所有的比值
...
# 创建树
def createTree(dataSet, label_s):
    ...
    bestFeatLabel = label_s[bestFeat]  # 根据下表找属性名称当树的根节点
    my_Tree = {bestFeatLabel: {}}  # 以bestFeatLabel为根节点建一个空树
    del (label_s[bestFeat])  # 从属性列表中删掉已经被选出来当根节点的属性
    ...
    for value in uniqueVals:  # 根据该属性的值求树的各个分支
        subLabels = label_s[:]
        my_Tree[bestFeatLabel][value] = createTree(splitDataSet(dataSet, bestFeat, value), subLabels)  # 根据各个分支递归创建树
    return my_Tree  # 生成的树
