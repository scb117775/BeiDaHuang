...
while True:
  ...
  if index == '1':
    try:
      ...
      #写入commodity_info_all  直接到最后所有都价格刷新结束后将商品价格加到总表
      # for i in range(len(foreign_id_1_1)):
      #   sql = 'insert into commodity_info_all ' \
      #         '(`foreign_id`,`商品名称`,`一级分类`,`二级分类`,`单位`,`品牌`) ' \
      #         'values (%s,%s,%s,%s,%s,%s)'
      #   val = (foreign_id_1_1[i],name_1_1[i],sort1_1_1[i],sort2_1_1[i],unit_1_1[i],brand_1_1[i])
      ...
      for i in range(len(foreign_id_2_3_1)):
        if foreign_id_2_3_1[i] not in foreign_id_yes:
          if price_buy_2_3_1[i] != '0':
            ...
            #   # 二级分类
            #   if rate_sort1[j] is not None and rate_sort2[j] is not None and rate_provider[j] is None:
            #     if rate_sort1[j] == sort1_2_3_1[i] and rate_sort2[j] == sort2_2_3_1[i]:
            #       rate_mid = float(rate[j])
            #       break
            #   # 一级分类
            #   if rate_sort1[j] is not None and rate_sort2[j] is None and rate_provider[j] is None:
            #     if rate_sort1[j] == sort1_1_2_2[i]:
            #       rate_mid = float(rate[j])
            #       break
      ...
      for i in range(len(foreign_id_2_3_2)):
        if foreign_id_2_3_2[i] not in foreign_id_yes:
          if price_buy_2_3_2[i] != '0':
            ...
            #   # 二级分类
            #   if rate_sort1[j] is not None and rate_sort2[j] is not None and rate_provider[j] is None:
            #     if rate_sort1[j] == sort1_2_3_2[i] and rate_sort2[j] == sort2_2_3_2[i]:
            #       rate_mid = float(rate[j])
            #       break
            #   # 一级分类
            #   if rate_sort1[j] is not None and rate_sort2[j] is None and rate_provider[j] is None:
            #     if rate_sort1[j] == sort1_2_3_2[i]:
            #       rate_mid = float(rate[j])
            #       break
    ...
    print('1.基础信息+源头采购价、平台进货价、售价 2.基础信息+源头采购价 3.基础信息+平台进货价 4.基础信息+售价 0.退出')
    ...
    if index1 == '1':
      ...
      try:
        ...
        file_out_path = input()
        df.to_excel(file_out_path + "/基础信息+源头采购价、平台进货价、售价.xlsx", index=False)
      except OSError:
        print('文件路径错误')
    elif index1 == '2':
      sql = "SELECT commodity.foreign_id 商品ID,commodity.`一级分类`,commodity.`二级分类`,commodity.`商品名称`,commodity.`单位`,commodity_price.`类型` 类别, " \
      ...
      try:
        print('请输入保存路径(. 保存到程序同级路径)')
        file_out_path = input()
        df.to_excel(file_out_path + "/基础信息+源头采购价.xlsx", index=False)
      except OSError:
        print('文件路径错误')
    elif index1 == '3':
      sql = "SELECT commodity.foreign_id 商品ID,commodity.`一级分类`,commodity.`二级分类`,commodity.`商品名称`,commodity.`单位`,commodity_price.`类型` 类别, " \
            "commodity_price.`平台进货价` FROM commodity " \
            "INNER JOIN commodity_price ON commodity.foreign_id = commodity_price.foreign_id"
      my_cursor.execute(sql)
      result = my_cursor.fetchall()
      # 将数据转换为pandas DataFrame
      ...
      try:
        print('请输入保存路径(. 保存到程序同级路径)')
        file_out_path = input()
        df.to_excel(file_out_path + "/基础信息+平台进货价.xlsx", index=False)
      except OSError:
        print('文件路径错误')
    elif index1 == '4':
      sql = "SELECT commodity.foreign_id 商品ID,commodity.`一级分类`,commodity.`二级分类`,commodity.`商品名称`,commodity.`单位`,commodity_price.`类型` 类别, " \
      ...
      try:
        print('请输入保存路径(. 保存到程序同级路径)')
        file_out_path = input()
        df.to_excel(file_out_path + "/基础信息+售价.xlsx", index=False)
      except OSError:
        print('文件路径错误')
