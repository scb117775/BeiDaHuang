...
while True:
  ...
  if index == '1':
    ...
    print('1、全部表导出  2.单个表导出')
    index3 = input('请输入导出选项：')
    if index3 == '1':
      print('导出中....')
      #4.1报价表
      ...
      # 将数据转换为pandas DataFrame
      df = pd.DataFrame(result, columns=[i[0] for i in my_cursor.description])
      df.to_excel(file_out_path + "/4.3平台进货价&服务费"+str(year)+'-'+str(month)+'-'+str(day)+".xlsx", index=False)
      ...
      print('1、 4.1报价表 2、 4.3平台进货价&服务费 3、 4.2平台销售价 0、 退出')
      index4 = input('请输入单个导出表的选项：')
      if index4 == '1':
        sql = "SELECT commodity.foreign_id 商品ID,commodity.`一级分类`,commodity.`二级分类`,commodity.`商品名称`,commodity.`单位`,commodity_price.`类型` 类别, " \
        ...
        # 将数据转换为pandas DataFrame
        df = pd.DataFrame(result, columns=[i[0] for i in my_cursor.description])
        df.to_excel(file_out_path + "/4.0源头采购价"+str(year)+'-'+str(month)+'-'+str(day)+".xlsx", index=False)
      elif index4 == '3':
        sql = "SELECT commodity.foreign_id 商品ID,commodity.`一级分类`,commodity.`二级分类`,commodity.`商品名称`,commodity.`单位`,commodity_price.`类型` 类别, " \
              "commodity_price.`平台进货价`,commodity_price.`配送服务费` FROM commodity " \
              "INNER JOIN commodity_price ON commodity.foreign_id = commodity_price.foreign_id"
